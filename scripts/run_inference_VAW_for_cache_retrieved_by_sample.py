...
from source.data import get_dataloader, MemoryMappedTensorDataset
...
def main(args):
    ...
    templates = load_templates_cache_generation(args.template_file)
    ...
    transform = transforms.Compose(
        [
            transforms.Resize(size=(args.image_size, args.image_size), interpolation=2),
            transforms.ToTensor(),
    ...
    dataset_image_embedding = MemoryMappedTensorDataset(os.path.join(config.DATASET_PATH, 'vocabulary_free', 'all_images', 'cc12m', f'{model_name}.npy'))
    ...
    with torch.no_grad():
        for i, (images, labels) in enumerate(tqdm(data_loader, desc='Batch')):
            ...
            if (image_features is None
                and any(arch in args.model_from.lower() for arch in ['x_vlm', 'blip'])
            ):
