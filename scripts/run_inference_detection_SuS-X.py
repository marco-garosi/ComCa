...
def main(args):
    ...
    with torch.no_grad():
        for i, (images, labels) in enumerate(tqdm(data_loader)):
            ...
            output = predictor(cropped_objects, scale_base_logits=args.scale_base_logits)
    ...
    pred_vectors = []
    all_image_features = torch.cat(test_features, dim=0)

    # Pre-loading all image features is necessary!
    test_features = all_image_features
    val_features = all_image_features  # actually not needed
    ...
    for idx in range(len(predictions)):
        for pred_idx in range(len(predictions[idx]['instances'])):
            predictions[idx]['instances'][pred_idx]['att_scores'] = pred_vectors[prediction_idx].tolist()
            prediction_idx += 1
