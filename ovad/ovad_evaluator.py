...
def print_metric_table(att_evaluator, results, file_print: str = '', print_table: bool = True):
    all_metrics = [
        "PCrand_P",
        "PCt{thr}_ap",
        # "PCt{thr}_R",
    ...
    for m in all_metrics:
        m_name = m.format(thr=f'{att_evaluator.threshold:.1f}', top_k=att_evaluator.top_k)
...
if __name__ == "__main__":
    ...
    results_table = {
        key: list(metrics.values()) for key, metrics in overall_metrics.items()
    }
    results_table["Type"] = [
        att_type.replace("PCt0.5_ap/", "")
        for att_type in overall_metrics["random"].keys()
    ]
